...
class GuestLocationLinkTest extends TestCase
{
    ...
    {
        $tenantA = Tenant::factory()->create(); $tenantB = Tenant::factory()->create();
    ...
    {
        $tenantA = Tenant::factory()->create(); $tenantB = Tenant::factory()->create();
        $adminA = User::factory()->adminRt($tenantA)->create(); $houseB = House::factory()->create(['tenant_id' => $tenantB->id, 'latitude' => -6.2, 'longitude' => 106.8]);
    ...
    {
        $tenant = Tenant::factory()->create(); $admin = User::factory()->adminRt($tenant)->create(); $house = House::factory()->create(['tenant_id' => $tenant->id]);
        foreach ([['latitude' => '90.0000000', 'longitude' => '180.0000000', 'valid' => true], ['latitude' => '90.1', 'longitude' => '106', 'field' => 'latitude'], ['latitude' => '-90.1', 'longitude' => '106', 'field' => 'latitude'], ['latitude' => '-6', 'longitude' => '180.1', 'field' => 'longitude'], ['latitude' => '-6', 'longitude' => '-180.1', 'field' => 'longitude'], ['latitude' => 'north', 'longitude' => 'east', 'field' => 'latitude']] as $coordinates) {
            ...
            if ($coordinates['valid'] ?? false) $response->assertRedirect(); else $response->assertSessionHasErrors($coordinates['field']);
        }
    }

    public function test_guest_token_is_hashed_and_public_page_is_limited_to_safe_location_data(): void
    {
        $tenant = Tenant::factory()->create(); $admin = User::factory()->adminRt($tenant)->create();
        ...
        $url = $response->getSession()->get('guest_location_url'); $token = basename($url);
        $link = GuestLocationLink::firstOrFail();
        $this->assertNotSame($token, $link->token_hash); $this->assertNotSame($token, $link->lookup_hash); $this->assertNotSame((string) $house->id, $token); $this->assertNotSame((string) $tenant->id, $token); $this->assertSame($tenant->id, $link->tenant_id); $this->assertSame($house->id, $link->house_id); $this->assertSame($admin->id, $link->created_by); $this->assertNull($link->revoked_at); $this->assertTrue($link->expires_at->lessThan(now()->addHours(25)));
    ...
    {
        $tenant = Tenant::factory()->create(); $admin = User::factory()->adminRt($tenant)->create(); $house = House::factory()->create(['tenant_id' => $tenant->id, 'latitude' => -6.2, 'longitude' => 106.8]);
        $service = app(GuestLocationLinkService::class);
        $expired = $service->generate($house, $admin); $expired->link->forceFill(['expires_at' => now()->subMinute()])->save();
        $this->get(route('guest.location.show', $expired->token))->assertNotFound();
        $revoked = $service->generate($house, $admin); $service->revoke($revoked->link);
    ...
    {
        $tenant = Tenant::factory()->create(); $house = House::factory()->create(['tenant_id' => $tenant->id, 'latitude' => -6.2, 'longitude' => 106.8]);
        $resident = Resident::factory()->create(['tenant_id' => $tenant->id]); $user = User::factory()->create(['tenant_id' => $tenant->id, 'resident_id' => $resident->id]);
    ...
    public function test_guest_location_route_enforces_its_rate_limit(): void
    {
        ...
        for ($attempt = 0; $attempt < 60; $attempt++) $this->get(route('guest.location.show', "invalid-token-{$attempt}"))->assertNotFound();
