...
class ResidentController extends Controller
{
    ...
    {
        return view('admin.residents.form', ['resident' => new Resident, 'tenant' => $currentTenant->forUser($request->user(), $request)]);
    }

    public function store(Request $request, CurrentTenant $currentTenant): RedirectResponse
    {
        $tenant = $currentTenant->forUser($request->user(), $request);
        $resident = new Resident($this->validated($request, $tenant->id));
        $resident->tenant()->associate($tenant);
        $resident->save();
    ...
    {
        Gate::authorize('manage', $resident);
    ...
    {
        Gate::authorize('manage', $resident);
        $data = $this->validated($request, $resident->tenant_id, $resident->id, false);
        foreach (['nik', 'phone'] as $field) {
            if (blank($data[$field] ?? null)) {
                unset($data[$field]);
            }
        } $resident->update($data);

        return to_route('admin.residents.show', $resident)->with('status', 'Data warga diperbarui.');
    }

    public function status(Request $request, Resident $resident): RedirectResponse
    {
        Gate::authorize('manage', $resident);
        $data = $request->validate(['status' => ['required', Rule::in(['active', 'inactive'])]]);
        $resident->update($data);

        return back()->with('status', 'Status warga diperbarui.');
    }
