...
<head>
    ...
    <title>@yield('title', 'RT Platform')</title>
...
<body class="antialiased min-h-screen flex bg-slate-50">
    ...
    <aside class="fixed inset-y-0 left-0 w-[260px] bg-white border-r border-slate-200 z-20 flex flex-col hidden lg:flex">
        ...
        <!-- Tenant Label -->
        <div class="px-6 py-4">
            <div class="bg-slate-50 border border-slate-200 rounded px-3 py-2 text-xs font-medium text-slate-700 flex flex-col">
                <span class="text-[10px] text-teal-700 font-bold mb-0.5 flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg> TENANT WILAYAH</span>
        ...
        <nav class="flex-1 overflow-y-auto px-4 space-y-6 pb-6">
            ...
            <div>
                ...
                <div class="space-y-1">
                    <a href="{{ route('dashboard') ?? '#' }}" class="flex items-center gap-3 px-3 py-2 @if(request()->routeIs('dashboard') || request()->is('dashboard') || request()->is('/')) bg-teal-700 text-white font-medium @else text-slate-600 hover:bg-slate-50 hover:text-slate-900 font-medium @endif rounded-md text-sm transition-colors">
            ...
            <div>
                ...
                <div class="space-y-1">
                    <a href="{{ route('warga.index') ?? '#' }}" class="flex items-center gap-3 px-3 py-2 @if(request()->routeIs('warga.*') || request()->is('warga')) bg-teal-700 text-white font-medium @else text-slate-600 hover:bg-slate-50 hover:text-slate-900 font-medium @endif rounded-md text-sm transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                        Data Warga
                    </a>
                    <a href="{{ route('admin.households.index') }}" class="flex items-center gap-3 px-3 py-2 text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-md text-sm font-medium transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                        Data Keluarga
                    </a>
                    <a href="{{ route('admin.houses.index') }}" class="flex items-center gap-3 px-3 py-2 text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-md text-sm font-medium transition-colors">
        ...
        <!-- Profile Bottom (optional, normally in header but design system has it in header) -->
        <div class="mt-auto px-6 py-4 border-t border-slate-200">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-full bg-teal-100 text-teal-700 flex items-center justify-center font-bold text-sm shrink-0">
                    H
                </div>
    ...
    <div class="flex-1 flex flex-col min-h-screen lg:pl-[260px]">
        ...
        <header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-10">
            ...
            <div class="flex items-center gap-4 pl-4">
                ...
                <div class="hidden sm:flex items-center gap-2 text-sm text-slate-500 font-medium border border-slate-200 rounded-md px-3 py-1.5 bg-slate-50">
                ...
                <div class="flex items-center gap-3 pl-2 border-l border-slate-200">
                    <div class="w-8 h-8 rounded-full bg-teal-700 text-white flex items-center justify-center font-bold text-sm">
                        H
                    </div>
        ...
        <!-- Main Workspace -->
        <main class="flex-1 p-6 lg:p-8 max-w-[1600px] w-full">
            @yield('content')
        </main>
    </div>
